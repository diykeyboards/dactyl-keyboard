...
from clusters.velvet import VelvetCluster
...
def make_dactyl():
    ...
    override_path = path.join(r".", "overrides")
    save_name = None
    ...
    if data["overrides"] not in [None, ""]:
        override_file = path.join(override_path, data["overrides"] + '.json')
    ...
    # Generate save_name
    if save_name != '':
        save_name = "DM_" + str(nrows) + "x" + str(ncols) + "_" + \
            plate_style + "_" + thumb_style + "_" + oled_mount_type

    # Replace with imported data name, if present
    if data["config_name"] != '':
        save_name = data["config_name"]

    # Prefix with overrides name, if present
    if overrides != '':
        save_name = overrides + "_" + save_name

    # Check for ENGINE, set if not present from configuration.
    ENGINE = data["ENGINE"]
    try:
    ...
    # Derived values
    if plate_style in ['NUB', 'HS-NUB']:
        keyswitch_height = nub_keyswitch_height
        keyswitch_width = nub_keyswitch_width
    elif plate_style in ['UNDERCUT', 'HS-UNDERCUT', 'NOTCH', 'HS-NOTCH']:
    ...
    elif 'HS-' in plate_style:
        symmetry = "asymmetric"
        pname = r"hot_swap_plate"
        plate_thickness = plate_thickness_hotswap
    ...
    def single_plate(cylinder_segments=100, side="right"):
        ...
        if plate_style in ['UNDERCUT', 'HS-UNDERCUT', 'NOTCH', 'HS-NOTCH', 'AMOEBA']:
            if plate_style in ['UNDERCUT', 'HS-UNDERCUT']:
                undercut = box(
                    keyswitch_width + 2 * clip_undercut,
                    keyswitch_height + 2 * clip_undercut,
                    mount_thickness - clip_thickness
                )

            if plate_style in ['NOTCH', 'HS-NOTCH', 'AMOEBA']:
                undercut = box(
                    notch_width,
                    keyswitch_width + 2 * clip_undercut,
                    mount_thickness - clip_thickness
                )
            if plate_style in ['NOTCH', 'AMOEBA']:
            ...
            if ENGINE == 'cadquery' and undercut_transition > 0:
                undercut = undercut.faces(
                    "+Z").chamfer(undercut_transition, clip_undercut)

            plate = difference(plate, [undercut])

        if plate_file is not None and 'HS-' in plate_style:
    ...
    def trackball_socket(btus=False, segments=100, side="right"):
        ...
        if btus:
            ...
            tbcut_file = path.join(parts_path, r"cutting_cylinder_44mm")
    ...
    def sa_cap(Usize=1):
        ...
        k1 = polyline([(bw2, bl2), (bw2, -bl2),
                      (-bw2, -bl2), (-bw2, bl2), (bw2, bl2)])
        k1 = extrude_poly(outer_poly=k1, height=0.1)
        k1 = translate(k1, (0, 0, 0.05))
        k2 = polyline([(pw2, pl2), (pw2, -pl2),
                      (-pw2, -pl2), (-pw2, pl2), (pw2, pl2)])
    ...
    def key_pcb():
        ...
        holes = [
            translate(hole, (pcb_hole_pattern_width / 2,
                      pcb_hole_pattern_height / 2, 0)),
            translate(hole, (-pcb_hole_pattern_width / 2,
                      pcb_hole_pattern_height / 2, 0)),
            translate(hole, (-pcb_hole_pattern_width /
                      2, -pcb_hole_pattern_height / 2, 0)),
            translate(hole, (pcb_hole_pattern_width /
                      2, -pcb_hole_pattern_height / 2, 0)),
        ]
    ...
    def apply_key_geometry(
        ...
        if column_style == "ORTHOGRAPHIC":
            ...
            shape = translate_fn(
                shape, [-(column - centercol) *
                        column_x_delta_actual, 0, column_z_delta]
            )
            ...
            shape = translate_fn(
                shape, [0, 0, -(row_radius + fixed_z[column])])
            shape = rotate_x_fn(shape, alpha * (centerrow - row))
    ...
    def web_post():
        debugprint('web_post()')
        post = box(post_size, post_size, plate_thickness)
        post = translate(post, (0, 0, plate_thickness - (plate_thickness / 2)))
        return post
    ...
    def adjustable_plate_half(Usize=1.5):
        ...
        top_plate = box(mount_width, adjustable_plate_height, plate_thickness)
        top_plate = translate(top_plate,
                              [0, (adjustable_plate_height + mount_height) /
                               2, plate_thickness - (plate_thickness / 2)]
                              )
        return top_plate
    ...
    def double_plate_half():
        debugprint('double_plate()')
        top_plate = box(mount_width, double_plate_height, plate_thickness)
        top_plate = translate(top_plate,
                              [0, (double_plate_height + mount_height) / 2,
    ...
    def left_key_position(row, direction, low_corner=False, side='right'):
        ...
        pos = np.array(
            key_position([-mount_width * 0.5, direction *
                         mount_height * 0.5, 0], 0, row)
        )
    ...
    def left_key_place(shape, row, direction, low_corner=False, side='right'):
        ...
        pos = left_key_position(
            row, direction, low_corner=low_corner, side=side)
    ...
    def back_wall():
        print("back_wall()")
        x = 0
        shape = union([key_wall_brace(x, 0, 0, 1, web_post_tl(),
                      x, 0, 0, 1, web_post_tr(), back=True)])
    ...
    def right_wall():
        ...
        if ncols > 4:
            shape = union([
                shape,
                key_wall_brace(lastcol, torow, 0, -1, web_post_br(),
                               lastcol, torow, 1, 0, web_post_br())
            ])
        return shape
        ...
        for i in range(lastrow):
            ...
            temp_shape1 = wall_brace(
                ...
                (lambda sh: left_key_place(sh, y, -1,
                 low_corner=low, side=side)), -1, 0, web_post(),
            )
    ...
    rj9_start = list(
        ...
        + np.array(
            key_position(
                list(np.array(wall_locate3(0, 1)) +
    ...
    def rj9_holder():
        print('rj9_holder()')
        shape = union([translate(box(10.78, 9, 18.38), (0, 2, 0)),
    ...
    usb_holder_position = key_position(
        list(np.array(wall_locate2(0, 1)) +
             np.array([0, (mount_height / 2), 0])), 1, 0
    )
    ...
    def blackpill_mount_hole():
        ...
        undercut = box(blackpill_holder_width + 8, 10.0,
    ...
    def external_mount_pads():
        print('external_mount_pads()')
        pad_l = translate(cylinder(radius=7.5, height=0.4),
                          ((-external_holder_width / 2) + 5, 0, 0)
                          )
        pad_r = translate(cylinder(radius=7.5, height=0.4),
                          (-5 + (external_holder_width / 2), 0, 0)
                          )
        shape = union([pad_l, pad_r])

        shape = translate(shape,
                          (
                              external_start[0] + external_holder_xoffset,
                              external_start[1] + external_holder_yoffset + 2,
                              0,
                          )
                          )
        return shape
    ...
    def generate_trackball(pos, rot, cluster):
        ...
        if cluster is not None and get_extras is True:
    ...
    def tbiw_position_rotation():
        base_pt1 = key_position(
            list(np.array([-mount_width / 2, 0, 0]) +
                 np.array([0, (mount_height / 2), 0])),
        ...
        angle_x = np.arctan2(
            base_pt1[2] - base_pt2[2], base_pt1[1] - base_pt2[1])
        angle_z = np.arctan2(
            base_pt1[0] - base_pt2[0], base_pt1[1] - base_pt2[1])
        tbiw_mount_rotation_xyz = (rad2deg(angle_x), 0, rad2deg(
            angle_z)) + np.array(tbiw_rotation_offset)
    ...
    def oled_position_rotation(side='right'):
        ...
        if _oled_center_row is not None:
            base_pt1 = key_position(
                list(np.array([-mount_width / 2, 0, 0]) + np.array([0,
                     (mount_height / 2), 0])), 0, _oled_center_row - 1
            )
            base_pt2 = key_position(
                list(np.array([-mount_width / 2, 0, 0]) + np.array([0,
                     (mount_height / 2), 0])), 0, _oled_center_row + 1
            ...
            oled_mount_location_xyz[2] = (
                oled_mount_location_xyz[2] + base_pt0[2]) / 2
            ...
            if trackball_in_wall and (side == ball_side or ball_side == 'both'):
                ...
                oled_mount_rotation_xyz = (0, rad2deg(
                    angle_x), -90) + np.array(_oled_rotation_offset)
            else:
    ...
    def oled_sliding_mount_frame(side='right'):
        ...
        top_hole_start = -mount_ext_height / 2.0 + oled_mount_rim + \
        ...
        conn_hole = box(oled_mount_width,
                        conn_hole_length + .01, oled_mount_depth)
        ...
        end_hole = box(oled_mount_width,
                       end_hole_length + .01, oled_mount_depth)
        ...
        top_hole_start = -mount_ext_height / 2.0 + oled_mount_rim + \
            oled_edge_overlap_end + oled_edge_overlap_connector
        top_hole_length = oled_mount_height
        top_hole = box(oled_mount_width, top_hole_length,
        ...
        top_chamfer_1 = translate(
            top_chamfer_1, (0, 0, -oled_edge_chamfer - .05))
    ...
    def oled_clip_mount_frame(side='right'):
        ...
        shape = box(mount_ext_width, mount_ext_height, oled_mount_depth)
        shape = difference(
            shape, [box(oled_mount_width, oled_mount_height, oled_mount_depth + .1)])
        ...
        clip_undercut = box(
            oled_clip_width + 2 * oled_clip_width_clearance,
            oled_mount_height + 2 * oled_clip_thickness + 2 *
            oled_clip_overhang + 2 * oled_clip_undercut,
            oled_mount_depth + .1
        )

        clip_undercut = translate(
            clip_undercut, (0., 0., oled_clip_undercut_thickness))
    ...
    def oled_clip():
        ...
        shape = difference(
            shape, [translate(hole, (0., 0., oled_mount_bezel_thickness))])
        ...
        latch_2 = translate(latch_2, (
            0.,
            -(-oled_clip_thickness / 2 +
              oled_clip_thickness + oled_clip_overhang) / 2,
        ...
        clip_leg = translate(clip_leg, (
            0.,
            (oled_mount_height + 2 * oled_clip_overhang +
             oled_clip_thickness) / 2 - oled_clip_y_gap,
            0.
        ))
    ...
    def oled_undercut_mount_frame(side='right'):
        ...
        undercut = box(
            ...
            oled_mount_depth)
        undercut = translate(
            undercut, (0., 0., -oled_mount_undercut_thickness))
    ...
    def teensy_holder():
        ...
        teensy_holder_top_offset = (
            teensy_holder_top_length / 2) - teensy_holder_length
        ...
        shape = translate(shape,
                          [teensy_top_xy[0], teensy_top_xy[1] -
                              1, (6 + teensy_width) / 2]
                          )
    ...
    def screw_insert_shape(bottom_radius, top_radius, height):
        ...
        if bottom_radius == top_radius:
            ...
            base = translate(cone(r1=bottom_radius, r2=top_radius,
    ...
    def screw_insert_all_shapes(bottom_radius, top_radius, height, offset=0, side='right'):
        ...
        shape = (
            translate(screw_insert(0, 0, bottom_radius, top_radius, height,
                      side=side), (so[0][0], so[0][1], so[0][2] + offset)),  # rear left
            ...
            translate(screw_insert(3, 0, bottom_radius, top_radius, height,
                      side=side), (so[3][0], so[3][1], so[3][2] + offset)),  # rear middle
            ...
            translate(screw_insert_thumb(bottom_radius, top_radius, height, side),
    ...
    def wire_post(direction, offset):
        ...
        s2 = translate(s2,
                       [0, -wire_post_overhang * 0.5 *
                           direction, -wire_post_height / 2]
                       )
    ...
    def model_side(side="right"):
        ...
        if debug_exports:
            export_file(shape=shape, fname=path.join(
                r"..", "things", r"debug_key_plates"))
        connector_shape = connectors()
        shape = union([shape, connector_shape])
        if debug_exports:
            export_file(shape=shape, fname=path.join(
                r"..", "things", r"debug_connector_shape"))
        thumb_shape = cluster(side).thumb(side=side)
        if debug_exports:
            export_file(shape=thumb_shape, fname=path.join(
                r"..", "things", r"debug_thumb_shape"))
        ...
        if debug_exports:
            export_file(shape=shape, fname=path.join(
                r"..", "things", r"debug_thumb_connector_shape"))
        walls_shape = case_walls(side=side)
        if debug_exports:
            export_file(shape=walls_shape, fname=path.join(
                r"..", "things", r"debug_walls_shape"))
        ...
        if controller_mount_type in ['EXTERNAL']:
            ...
            s2 = union([s2, external_mount_pads()])
    ...
    def wrist_rest(base, plate, side="right"):
        rest = import_file(
            path.join(parts_path, "dactyl_wrist_rest_v3_" + side))
    ...
    def baseplate(wedge_angle=None, side='right'):
        ...
        if ENGINE == 'cadquery':
            ...
            outside = shape.vertices(cq.DirectionMinMaxSelector(
                cq.Vector(1, 0, 0), True)).objects[0]
            sizes = []
            max_val = 0
            inner_index = 0
            ...
            if wedge_angle is not None:
                cq.Workplane('XY').add(cq.Solid.revolve(
                ...
                inner_shape = translate(
                    inner_shape, (0, 0, -base_rim_thickness))
                inner_outline = cq.Workplane('XY').add(
                    cq.Solid.extrudeLinear(inner_wire, [], cq.Vector(0, 0, base_rim_thickness)))

                if logo_file not in ["", "None", None]:
                ...
                for hole in holes:
                    ...
                    hole_shapes.append(
                        translate(
                            cylinder(screw_hole_diameter /
                                     2.0, base_rim_thickness),
                            (loc.x, loc.y, 0)
                        )
                    )
                bore_shapes = []
                ...
                for hole in holes:
                    ...
                    if screw_cbore_style == "COUNTERBORE":
                        bore_shapes.append(
                            translate(
                                cylinder(screw_cbore_diameter /
                                         2.0, screw_cbore_depth),
                                (loc.x, loc.y, 0)
                            )
                        )
                    if screw_cbore_style == "COUNTERSINK":
                        bore_shapes.append(
                            translate(
                                cone(screw_cbore_diameter/2.0,
                                     screw_hole_diameter/2.0, screw_cbore_depth),
                                (loc.x, loc.y, 0)
                            )
                        )

                # plate_shape = difference(shape, hole_shapes)
                plate_shape = difference(shape, bore_shapes)
                plate_shape = translate(
                    plate_shape, (0, 0, -base_rim_thickness))
                plate_shape = union([plate_shape, inner_shape])

                plate_outline = difference(shape, hole_shapes)
                plate_outline = union([plate_outline, inner_outline])

            return plate_shape, plate_outline
            ...
            tool = translate(
                union(screw_insert_screw_holes(side=side)), [0, 0, -10])
    ...
    def run():
        ...
        export_file(shape=mod_r, fname=path.join(
            save_path, save_name + r"_Right_Case"))

        base_r, base_r_outline = baseplate(side='right')
        export_file(shape=base_r, fname=path.join(
            save_path, save_name + r"_Right_Plate"))
        export_dxf(shape=base_r_outline, fname=path.join(
            save_path, save_name + r"_Right_Plate"))
        ...
        #
        # export_file(shape=rest, fname=path.join(save_path, save_name + r"_right_wrist_rest"))

        # if symmetry == "asymmetric":

        mod_l = model_side(side="left")
        export_file(shape=mod_l, fname=path.join(
            save_path, save_name + r"_Left_Case"))

        base_l, base_l_outline = baseplate(side='left')
        base_l = mirror(base_l, 'YZ')
        base_l_outline = mirror(base_l_outline, 'YZ')
        export_file(shape=base_l, fname=path.join(
            save_path, save_name + r"_Left_Plate"))
        export_dxf(shape=base_l_outline, fname=path.join(
            save_path, save_name + r"_Left_Plate"))

        # else:
        #     export_file(shape=mirror(mod_r, 'YZ'), fname=path.join(save_path, save_name + r"_left"))
        #
        #     lbase = mirror(base, 'YZ')
        #     export_file(shape=lbase, fname=path.join(save_path, save_name + r"_left_plate"))
        #     export_dxf(shape=lbase, fname=path.join(save_path, save_name + r"_left_plate"))
        ...
        if test == True:
            ...
            if oled_mount_type == 'CLIP':
                ...
                export_file(shape=oled_clip(), fname=path.join(
                    save_path, config_name + r"_OLED_Clip"))
    ...
    def get_cluster(style):
        ...
        elif style == VelvetCluster.name():
            clust = VelvetCluster(all_merged)
    ...
    #    left_cluster = get_cluster(other_thumb)
        ...
        # this assumes thumb_style always overrides DEFAULT other_thumb
        left_cluster = right_cluster
